...
def drop_cols_na(df, num_rows, miss_p_thresh, verbose=False):
    cols_to_keep = []
    for c in df.columns:
    ...
    if verbose:
        print(f"Keeping {num_cols_kept} columns out of {num_cols} w over {miss_p_thresh} values non-missing")
        print(cols_to_keep)

    df_col_lim = df[cols_to_keep]
    return df_col_lim

def drop_rows_na(df, num_cols, miss_p_thresh, verbose=False):
    rows_to_keep = []
    for idx, row in df.iterrows():
        if rows_missing_count[idx]/num_cols_kept < miss_p_thresh:
            rows_to_keep.append(idx)

    num_rows_kept = len(rows_to_keep)
    if verbose:
        print(f"Keeping {num_rows_kept} columns out of {num_rows} w over {MISS_P_THRESH} values non-missing")
...
def clean_null_data(df, miss_p_thresh=MISS_P_THRESH, verbose=False):
    ...
    # Remove columns
    df = drop_cols_na(df, num_rows, miss_p_thresh, verbose)
    if verbose:
        print("Removed columns with too many null values.")

    # Remove rows
    df = drop_rows_na(df, num_cols, miss_p_thresh, verbose)
    if verbose:
